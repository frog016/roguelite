...
namespace Edgar.Unity.Editor
{
    public static class GridUtils
    {
        ...
        public static void DrawRectangleOutline(Grid grid, Vector3Int fromTile, Vector3Int toTile, Color color, Vector2 sizeModifier = default, bool addDiagonal = false, string label = null, bool isDotted = false)
        {
            ...
            var xDirection = grid.CellToLocalInterpolated(Vector2.right);
            var yDirection = grid.CellToLocalInterpolated(Vector2.up);

            var xSizeModifier = grid.CellToLocalInterpolated(new Vector3(sizeModifier.x, 0));
            var ySizeModifier = grid.CellToLocalInterpolated(new Vector3(0, sizeModifier.y));
